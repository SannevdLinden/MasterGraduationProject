...
import psycopg2
import pandas as pd
from step1_remove_qual_data import remove_qual_from_quant
from bloatectomy import bloatectomy #lib for redundancy 
...
#remove redundancy, need to do this once to create the file
def remove_redundancy(data, subject_id):
    tmp = '' #needs a string as input
    #concatenate all notes from one cluster to one doc with identifiers to recognize the individual notes
    for index, row in data.iterrows():
        tmp += 'Doc'+ str(index) + '. ' + row['text'] #every note has an identifier "Doc<number>. "
    
    bloatectomy(tmp,
            style='highlight', display=False,
            filename='./patient_data/redundancy_removed_' + subject_id,
            output='html',
...
if __name__ == "__main__":
    ...
    remove_redundancy(data, subject_id) 
    #split the concatenated files back into individual notes where duplicated information is marked  
    marked_tokens = codecs.open('./patient_data/redundancy_removed_' + subject_id + '_token_numbers.txt', 'r') 
    marked_tokens = marked_tokens.read()
    marked_tokens = marked_tokens.splitlines()
    marked_count = 0
    total_count = 0
    #get if duplicated text is above 15% of all text otherwise do not mark redundancy
    for line in marked_tokens:
        total_count += len(line)
        if(re.match(r'\d+\$ <mark>', line)):
            marked_count += len(line)
    percentage_dup = (marked_count * 100)/ total_count
    if(percentage_dup > 15):
        redundancy_file = codecs.open('./patient_data/redundancy_removed_' + subject_id + '.html', 'r')
        tmp = redundancy_file.read()
        for index, row in data.iterrows():
            if (index + 1):
                del1 = 'Doc'+ str(index) + '.'
                del2 = 'Doc'+ str(index + 1) + '.'
                text = tmp.split(del1)
                text = text[1].split(del2)
                data.at[index, 'text'] = text[0]
            else: 
                text = tmp.split('Doc'+ str(index) + '.')
                data.at[index, 'text'] = text[1]
