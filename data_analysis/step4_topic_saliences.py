...
def makeTree(CUIset, ICD10_bool):
    ...
    root = Node('root', leavesCount = 0, occurence = 0, indexArray = None, idNode='root')
    for x in CUIset: 
        ...
        if(len(ancestors) > 0): #sometimes concepts not recognized 
            ...
            leaveExists = search.find_by_attr(root, ancestors[0][0].name) #search if leave is new
            for index, ancestor in reversed(list(enumerate(ancestors[0]))): #first the highest ancestor to walk down path of this term
                ...
                if (searchNode): #node already exist
                    ...
                    if (not leaveExists): # if there is a new leave then add leave count
                        searchNode.leavesCount += 1
                else: #new leave
                    if (index == len(ancestors[0]) -1 ): #last element in array, child of root
                        parentNode = root                        
                        vars()[name] = Node(name, parent = parentNode, leavesCount = 1, occurence = 1, indexArray = None, idNode=name)
                    else:
                        parentNode = ancestors[0][index + 1].name
                        if(index == 0): #if it is a leave add the index in de original array
                            vars()[name] = Node(name, parent = vars()[parentNode], leavesCount = 1, occurence = 1, indexArray = x[0], idNode=name)
                        else: 
                            vars()[name] = Node(name, parent = vars()[parentNode], leavesCount = 1, occurence = 1, indexArray = None, idNode=name)
    return(root, CUIset)
    
#detect 5 most salient terms of the tree
def salience_tree(tree):
    result = [] #top 5 salient terms
    percentages = [] #percentages of the children of the root, based upon how many leaves each subtree with
    ...
    for node in children_root:
        total_leaves += node.leavesCount
    if (total_leaves <= 5): #if there are only 5 leaves or less just take them all
        for leave in tree.leaves:
            result.append(leave.indexArray)
        return result
    else: 
        counter = 0
        # calculate percentages of children root
        for child in children_root:
            percentage = (child.leavesCount * 100 ) / total_leaves
        ...
        while counter < 5: #as long as their have not been chosen 5 salient terms
            ...
            while not node.is_leaf:
                ...
                #on every level pick child with highest leave count
                children = np.array(node.children)
                #if multiple children have the same leave count a random one will be chosen
                np.random.shuffle(children) 
                max_value = 0
                for child in children:
                    if child.leavesCount > max_value:
                        node = child
                        max_value = child.leavesCount            
            result.append(node.indexArray)
            #get all ancestors of chosen leave
            ancestors = node.ancestors
            #decrease count so same leaves are not chosen again
            for ancestor in reversed(ancestors):
                if (ancestor != tree.root):
                    ancestor.leavesCount -= 1 
...
def topic_saliences(data, cluster, conceptExtraction, subject_id):   
    if (conceptExtraction):
        ...
        #for the disease category make the ICd-10 tree and extract terms from tree
        if(len(disease) > 5):
